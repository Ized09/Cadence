...
function SamplesStep({ samples, setSamples, onNext, busy }) {
  ...
  const addPaste = useCallback(() => {
    ...
    setSamples((s) => [...s, {
      _id: makeId(),
      source_type: "paste",
      title: pasteTitle || `Sample ${s.length + 1}`,
      raw_text: pasteText,
    }]);
    setPasteText("");
    setPasteTitle("");
  }, [pasteText, pasteTitle, setSamples]);
  ...
  const onFiles = useCallback(async (e) => {
    ...
    for (const f of files) {
      try {
        ...
        setSamples((s) => [...s, { _id: makeId(), source_type: "upload", title: f.name, raw_text: text }]);
      } catch (err) {
    ...
    <div className="animate-fade-up">
      ...
      <div className="surface p-6 mb-6">
        ...
        <div className="mt-3 flex items-center justify-between">
          <button data-testid="sample-add" onClick={addPaste} className="btn-primary">Add this sample</button>
          ...
          <div className="surface divide-soft">
            {samples.map((s) => (
              <div key={s._id} className="p-4 flex items-center justify-between">
                ...
                <button onClick={() => removeSample(s._id)} className="text-text-secondary hover:text-coral" data-testid={`sample-remove-${s._id}`}>
...
function AudienceStep({ audience, setAudience, onBack, onGenerate, busy }) {
  return (
    <div className="animate-fade-up">
      ...
      <div className="flex justify-between items-center">
        <button onClick={onBack} className="btn-ghost" data-testid="onboarding-back-2">← Back</button>
        <button
...
function VoiceStep({ profile, onRefine, refineText, setRefineText, onFinish, busy }) {
  return (
    <div className="animate-fade-up">
      ...
      <p className="text-text-secondary leading-relaxed mb-8 max-w-2xl">
        Read it. If something&apos;s off, tell me in plain language (&ldquo;less corporate,&rdquo; &ldquo;use more sentence fragments&rdquo;). I&apos;ll rewrite it.
      </p>
      ...
      <div className="surface p-5 mb-8">
        ...
        <div className="flex gap-3">
          ...
          />
          <button data-testid="refine-btn" disabled={busy || !refineText.trim()} onClick={onRefine} className="btn-ghost">
            {busy ? "…" : "Revise"}
          </button>
        </div>
      </div>
      ...
      <div className="flex justify-end">
        <button data-testid="onboarding-finish" onClick={onFinish} className="btn-primary inline-flex items-center gap-2">
...
export default function Onboarding() {
  ...
  const goToStep2 = useCallback(async () => {
    ...
    try {
      for (const s of samples) {
        const { _id, ...rest } = s;
        await api.post("/voice/samples", rest);
      }
      setStep(2);
    } catch (e) {
      console.error("save samples failed", e);
  ...
  const generate = useCallback(async () => {
    ...
    try {
      ...
      console.error("generate profile failed", e);
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-3xl mx-auto px-6 py-12 md:py-16">
        {step === 1 && <SamplesStep samples={samples} setSamples={setSamples} onNext={goToStep2} busy={busy} />}
        {step === 2 && <AudienceStep audience={audience} setAudience={setAudience} onBack={() => setStep(1)} onGenerate={generate} busy={busy} />}
        {step === 3 && profile && (
          <VoiceStep
            profile={profile}
            onRefine={refine}
