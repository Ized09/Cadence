...
export default function Review() {
  ...
  const scheduleNudge = async () => {
    setBusy(true);
    try {
      const when = new Date(Date.now() + reminderHours * 3600 * 1000).toISOString();
      await api.post(`/drafts/${id}/reminder`, { scheduled_for: when, channel: "email" });
      toast.success(`Nudge set for ${reminderHours}h from now.`);
      nav("/dashboard");
    } catch (e) {
  ...
  if (!draft) {
    return (<div className="min-h-screen"><Nav /><div className="max-w-3xl mx-auto px-6 py-10 label-mono">Loading…</div></div>);
  }
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-4xl mx-auto px-6 py-10">
        ...
        <div className="label-mono mb-10">{draft.body.split(/\s+/).filter(Boolean).length} WORDS · STATUS {draft.status.toUpperCase()}</div>
        ...
        {published ? (
          <div className="surface p-6 flex items-center gap-3" data-testid="published-banner">
            ...
            <div>
              ...
              <div className="label-mono mt-1">Sanity doc id: {draft.published_doc_id}</div>
            </div>
          </div>
        ) : (
          <div className="grid md:grid-cols-2 gap-4">
            <div className="surface p-6">
              ...
              {!target && (
                <div className="pill pill-coral mb-3" data-testid="target-missing">SANITY NOT CONNECTED — <Link to="/settings" className="underline ml-1">CONNECT</Link></div>
              )}
              <button disabled={busy || !target} onClick={publish} className="btn-primary inline-flex items-center gap-2 w-full justify-center disabled:opacity-40" data-testid="publish-btn">
            ...
            <div className="surface p-6">
              ...
              <p className="text-text-secondary text-sm mb-4">
                Set a nudge. You'll get an email when it's time. Draft moves to "ready · nudge set".
              </p>
              <div className="flex items-center gap-2 mb-3">
                <input type="number" min="1" max="240" value={reminderHours} onChange={(e) => setReminderHours(+e.target.value)} className="input-ink w-24" data-testid="reminder-hours" />
                <span className="label-mono">HOURS FROM NOW</span>
              </div>
              <button disabled={busy} onClick={scheduleNudge} className="btn-attention inline-flex items-center gap-2 w-full justify-center" data-testid="schedule-reminder-btn">
